...
pub fn gen_model(_attr: TokenStream, _item: TokenStream) -> TokenStream {
    ...
    let model_filter = q_model_filter(filter.clone(), filter_fields, filter_matches);
    let model_order_by = q_model_order_by(order_by.clone(), order_by_fields, order_by_matches);
    let model_search = q_model_search(alias.clone(), filter.clone(), order_by, search_fn);
    let model_detail = q_model_detail(alias.clone(), detail_fn);
    let model_delete = q_model_delete(alias.clone(), delete_fn);
