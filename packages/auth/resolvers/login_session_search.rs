...
pub(crate) async fn login_session_search_impl(
    ctx: &Context<'_>,
    ...
    let tx = &*ctx.tx().await?;
    let extra = get_session_filter(ctx).await?;
...
pub(crate) async fn login_session_count_impl(
    ctx: &Context<'_>,
    filter: Option<LoginSessionFilter>,
) -> Res<u64> {
    ctx.auth_ensure_authenticated().await?;
...
async fn get_session_filter(ctx: &Context<'_>) -> Res<LoginSessionFilter> {
